...
export default function applyEmailHandler(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === "POST") {
        ...
        const msg = {
            to: req.body.recruiterData.user_email,
            ...
            <br>
            ご不明な点などありましたら、 ${process.env.ADMIN_EMAIL_ADDRESS} (運営)
            もしくは ${process.env.SENDER_APPLY_EMAIL_ADDRESS} (応募メール担当者)までご連絡ください。</html>`,
        };
